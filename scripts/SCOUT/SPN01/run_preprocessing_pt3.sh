...
echo "singularity exec --bind /orfeo:/orfeo --no-home $image Rscript $base/preprocess_SAM.R ${cell_ids[$SLURM_ARRAY_TASK_ID-1+2000]}"
singularity exec --bind /orfeo:/orfeo --no-home $image Rscript $base/preprocess_SAM.R ${cell_ids[$SLURM_ARRAY_TASK_ID-1+2000]}
