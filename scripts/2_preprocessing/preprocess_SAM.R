...
merged_sam_dir <- paste0(outdir,"/MERGED_SAM")
dir.create(merged_sam_dir)
output_local_dir <- paste0(outdir,"/SAM")
chromosomes <-paste0("chr_",c(seq_along(1:22),"X","Y"))

merge_sams(output_local_dir = output_local_dir,BAM_file = file.path(merged_sam_dir,"merged.bam"),
...
splitted_bam_dir <- paste0(outdir,"/SPLITTED_BAM")
...
BAM_files <- list.files(splitted_bam_dir, pattern = "\\.bam$")
# 
...
fastq_dir <- paste0(outdir,"/FASTQ")
dir.create(fastq_dir)

for (c in BAM_files){
  curr_BAM_file <- file.path(splitted_bam_dir, c)
