...
#SBATCH --mem=100gb
#SBATCH --time=4:00:00
#SBATCH --output=Preprocess_BULK_normal_%A_%a.out
#SBATCH --error=Preprocess_BULK_normal_%A_%a.err
#SBATCH --job-name=Preprocess_BULK_normal
#SBATCH --array=1-2%2 ### change this with the number chunks you have for the normal sample
...
singularity exec --bind /orfeo:/orfeo --no-home $image Rscript "$base/scripts/2_preprocessing/preprocess_normal_SAM.R" $phylo_path $outdir $SLURM_ARRAY_TASK_ID
