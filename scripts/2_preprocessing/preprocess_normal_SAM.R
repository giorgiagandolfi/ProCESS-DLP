...
outdir_chunk <- paste0(outdir,"/normal_sample/sequencing_",chunk_name)
step=1
...
merge_sams(output_local_dir = outdir_chunk,BAM_file = file.path(outdir_chunk,paste0(chunk_name,".bam")),
...
BAM_files <- list.files(outdir_chunk, pattern = "\\.bam$")
# 
...
for (c in BAM_files){
  curr_BAM_file <- file.path(outdir_chunk,c)
  generate_fastq(curr_BAM_file, outdir_chunk,remove_local_bam=T)
}
